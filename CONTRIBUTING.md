...
3. **Build cleanly**: `xcodebuild ... build` should produce zero warnings.
...
The repository has GitHub Copilot Code Review enabled — expect an automated review pass within a few minutes of opening the PR. Please address or explicitly acknowledge each comment.
...
The current hero asset is `docs/demo.gif`. Additional visuals are welcome:

- **Replace `docs/demo.gif`** with a sharper / shorter recording (keep the same filename so the README picks it up).
- **Add a still screenshot** at `docs/screenshot.png` if you want one alongside the GIF — uncomment a second `<img>` line in the README.
...
- Tools that work well: [Kap](https://getkap.co/), [LICEcap](https://www.cockos.com/licecap/), `Cmd+⇧+5` (macOS built-in recorder → convert with `ffmpeg`).
